...
from .configuration_pi0_drift import DEFAULT_IMAGE_SIZE, PI0DriftConfig
...
class PI0DriftPytorch(nn.Module):  # see openpi `PI0DriftPytorch`
    """Core PI0 PyTorch model."""

    def __init__(self, config: PI0DriftConfig, rtc_processor: RTCProcessor | None = None):
        super().__init__()
        self.config = config
    ...
    def gradient_checkpointing_enable(self):
        ...
        logging.info("Enabled gradient checkpointing for PI0DriftPytorch model")
    ...
    def gradient_checkpointing_disable(self):
        ...
        logging.info("Disabled gradient checkpointing for PI0DriftPytorch model")
    ...
    def _build_drift_prefix_cache(self, images, img_masks, lang_tokens, lang_masks):
        prefix_embs, prefix_pad_masks, prefix_att_masks = self.embed_prefix(
            images, img_masks, lang_tokens, lang_masks
        )
        attention = prepare_attention_masks_4d(make_att_2d_masks(prefix_pad_masks, prefix_att_masks))
        ...
        if cache is None:
            raise RuntimeError("PI0-Drift prefix prefill did not return a KV cache.")
        return prefix_pad_masks, cache

    @staticmethod
    def _expand_drift_context(state, prefix_pad_masks, cache, samples):
        if samples == 1:
            return state, prefix_pad_masks, cache
        expanded_cache = DynamicCache(
            tuple(
                (
                    keys.repeat_interleave(samples, dim=0),
                    values.repeat_interleave(samples, dim=0),
                    sliding_window,
                )
        ...
        return (
            ...
            prefix_pad_masks.repeat_interleave(samples, dim=0),
            expanded_cache,
        )

    def _drift_direct_read(self, noise, state, prefix_pad_masks, cache):
        fixed_time = torch.ones(noise.shape[0], dtype=torch.float32, device=noise.device)
    ...
    def _sample_drift_candidates(self, samples, state, prefix_pad_masks, cache):
        batch_size = state.shape[0]
        expanded_state, expanded_pad, expanded_cache = self._expand_drift_context(
            state, prefix_pad_masks, cache, samples
        )
        noise = self.sample_noise(
            (batch_size * samples, self.config.chunk_size, self.config.max_action_dim), state.device
        )
        generated = self._drift_direct_read(noise, expanded_state, expanded_pad, expanded_cache)
        return generated.view(batch_size, samples, self.config.chunk_size, self.config.max_action_dim)
    ...
    def forward_drifting(
        ...
        if action_feature is None:
            raise ValueError("PI0-Drift requires an action feature definition.")
        real_dim = action_feature.shape[0]
        prefix_pad, cache = self._build_drift_prefix_cache(images, img_masks, lang_tokens, lang_masks)
        generated = self._sample_drift_candidates(
            self.config.drifting_gen_per_label, state, prefix_pad, cache
        )[..., :real_dim]
        demonstrations = actions[..., :real_dim]
        valid = (
            torch.ones(batch_size, self.config.chunk_size, dtype=torch.bool, device=state.device)
        ...
        with torch.no_grad():
            ...
            values.extend(
                [
                    ((generated.mean(dim=1) - demonstrations).square() * mask).sum() / denom,
                    (generated.var(dim=1, unbiased=False) * mask).sum() / denom,
                ]
            )
    ...
    def sample_actions_drifting(
        ...
        prefix_pad, cache = self._build_drift_prefix_cache(images, img_masks, lang_tokens, lang_masks)
        samples = self.config.test_time_samples if noise is None else 1
        if samples > 1:
            candidates = self._sample_drift_candidates(samples, state, prefix_pad, cache)
            ...
            return candidates[torch.arange(state.shape[0], device=state.device), indices]
        if noise is None:
            noise = self.sample_noise(
                (state.shape[0], self.config.chunk_size, self.config.max_action_dim), state.device
            )
        return self._drift_direct_read(noise, state, prefix_pad, cache)


class PI0DriftPolicy(PreTrainedPolicy):
    """PI0 OpenPI Policy for LeRobot."""

    config_class = PI0DriftConfig
    name = "pi0_drift"
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.model = PI0DriftPytorch(config, rtc_processor=self.rtc_processor)
    ...
    def forward(self, batch: dict[str, Tensor], reduction: str = "mean") -> tuple[Tensor, dict]:
        ...
        if self.config.use_drifting_loss:
            ...
            loss_dict = {f"drift_{key}": value for key, value in drift_info.items()}
            loss_dict["loss"] = loss.mean().item() if reduction == "none" else loss.item()
